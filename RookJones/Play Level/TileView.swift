...
struct TileView: View {
    ...
    private func hasPieceImage() -> Bool {
        return pieceTileNameForType() != nil
    }
...
struct CellView_Previews: PreviewProvider {
	static var previews: some View {
		Group {
			Group {
				TileView(model: TileModel(.BlackRook))
					.previewDisplayName("Rook")
				TileView(model: TileModel(.Exit))
					.previewDisplayName("Open")
				TileView(model: TileModel(.WhiteQueen))
			...
			Group {
                ...
                TileView(model: TileModel(.RookJones))
